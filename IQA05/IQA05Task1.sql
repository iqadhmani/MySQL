...
SELECT 
    FORMAT(list_price, 2) AS price_format,
    CAST(discount_percent AS UNSIGNED) AS discount_cast,
...
SELECT 
    ...
    CONCAT('XXXX-XXXX-XXXX-',
            '',
            RIGHT(card_number, 4)) AS formatted_number
FROM
    orders;
...
FROM
    orders
WHERE
    DATE_FORMAT(order_date, '%M %Y') = 'April 2015';